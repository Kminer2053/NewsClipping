...
- 오직 최종 브리핑 결과만 출력합니다.
- **기사 원문 충실성**: 각 뉴스 상세페이지의 기사내용은 반드시 기사 원문의 내용을 정확하고 충실하게 반영해야 합니다. 원문에 없는 내용을 추가하거나, 원문의 내용을 왜곡하거나 변경하지 마세요. 기사 원문의 사실과 논조를 그대로 유지하면서 요약하세요.
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (!inSummaryPage) {
            ...
            if (isPublisherName) {
                ...
                if (includeCurrentArticle) {
                    console.log('[필터링] 기사 포함:', { publisher: currentPublisher, title: currentTitle });
                    filteredLines.push(lines[i]);
                } else {
                    console.warn('[필터링] 기사 제외:', { publisher: currentPublisher, title: currentTitle });
                }
                continue;
            }
